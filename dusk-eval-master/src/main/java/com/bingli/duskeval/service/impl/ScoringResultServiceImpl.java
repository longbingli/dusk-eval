...
import com.bingli.duskeval.mapper.ScoringResultMapper;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        // 创建数据时，参数不能为空
        if (add) {
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR, "结果不能为空");
            ThrowUtils.throwIf(StringUtils.isBlank(resultDesc), ErrorCode.PARAMS_ERROR, "描述不能为空");
            ThrowUtils.throwIf(StringUtils.isBlank(resultProp), ErrorCode.PARAMS_ERROR, "属性不能为空");
        ...
        ThrowUtils.throwIf(resultName != null && resultName.length() > 8000, ErrorCode.PARAMS_ERROR, "结果过长");
    }
    ...
    public QueryWrapper<ScoringResult> getQueryWrapper(ScoringResultQueryRequest scoringResultQueryRequest) {
        ...
        queryWrapper.like(StringUtils.isNotBlank(resultDesc), "resultProp", resultProp);
        // 精确查询
